...
from django.conf.settings import DOCKER_URL, DOCKER_API_VERSION
...
def build_image(profile, clean=False, ignore_cache=False):
    ...
    with sh.cd(tmpd):
        sh.git('clone', profile.git_repository)
        ...
        dock.build(path=tmpd, tag=profile.identifier, quiet=True, nocache=ignore_cache)
